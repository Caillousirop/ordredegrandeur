...
export const useQuestionFiltering = ({
  ...
  useEffect(() => {
    ...
    console.log("🔄 [FILTER] Début du filtrage des questions");
    console.log("Theme sélectionné:", selectedTheme?.id);
    console.log("Type sélectionné:", selectedType);
    console.log("Questions totales disponibles:", supabaseQuestions.length);
    ...
    if (selectedTheme && selectedTheme.id !== "random" && selectedTheme.id !== "challenge-30s") {
      console.log(`🎯 [FILTER] Filtrage par thème: ${selectedTheme.id}`);
      filtered = filtered.filter(q => q.theme.toLowerCase() === selectedTheme.id.toLowerCase());
      console.log(`✅ [FILTER] Questions après filtrage thème: ${filtered.length}`);
    }
    
    // Special handling for challenge-30s theme
    if (selectedTheme && selectedTheme.id === "challenge-30s") {
      console.log("🏃 [FILTER] Mode challenge 30s - questions simples uniquement");
      filtered = filtered.filter(q => q.type === "simple");
      console.log(`✅ [FILTER] Questions challenge 30s: ${filtered.length}`);
    }
    ...
    if (selectedType !== "all" && selectedTheme?.id !== "challenge-30s") {
      console.log(`🔧 [FILTER] Filtrage par type: ${selectedType}`);
      filtered = filtered.filter(q => q.type === selectedType);
      console.log(`✅ [FILTER] Questions après filtrage type: ${filtered.length}`);
    }
    
    // For logged users, prioritize unanswered questions
    if (user && scores.length > 0) {
      console.log("👤 [FILTER] Utilisateur connecté - priorisation des questions non répondues");
      
      // Get IDs of answered questions
      const answeredQuestionIds = new Set(scores.map(score => score.questionId));
      console.log("📊 [FILTER] Questions déjà répondues:", answeredQuestionIds.size);
      ...
      console.log("❓ [FILTER] Questions non répondues:", unansweredQuestions.length);
      console.log("✅ [FILTER] Questions déjà répondues:", answeredQuestions.length);
      ...
      console.log("🔄 [FILTER] Questions réorganisées pour utilisateur connecté");
    } else if (selectedTheme && selectedTheme.id === "random") {
      // For random theme without user login, just shuffle
      filtered = filtered.sort(() => Math.random() - 0.5);
      console.log("🎲 [FILTER] Questions mélangées pour thème aléatoire");
    }
    
    console.log("🏁 [FILTER] Questions filtrées finales:", filtered.length);
    ...
    if (filtered.length > 0) {
      ...
      console.log("🎯 [FILTER] Question initiale définie - Index: 0, ID:", filtered[0].id);
    } else {
      setCurrentQuestionIndex(0);
      setCurrentQuestionId(null);
      console.log("⚠️ [FILTER] Aucune question disponible après filtrage");
    }
