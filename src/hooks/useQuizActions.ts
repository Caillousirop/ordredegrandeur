...
export const useQuizActions = ({
  ...
  const handleNext = useCallback(() => {
    console.log("🎯 [QUIZ] Passage à la question suivante demandé");
    if (filteredQuestions.length > 1) {
      ...
      console.log(`🎯 [QUIZ] Nouvelle question index: ${newIndex}`);
    } else if (filteredQuestions.length === 1) {
      setCurrentQuestionIndex(0);
      console.log("🎯 [QUIZ] Une seule question disponible, index 0");
    }
  }, [filteredQuestions, currentQuestionIndex, setCurrentQuestionIndex]);
  
  const handleScore = useCallback(async (score: QuizScore) => {
    console.log("🎯 [QUIZ] Début traitement du score:", score);
    
    // BLOQUER IMMÉDIATEMENT tout changement de question
    ...
    try {
      ...
      console.log("✅ [QUIZ] Traitement du score terminé");
    } catch (error) {
      console.error("❌ [QUIZ] Erreur lors du traitement du score:", error);
      toast.error("Erreur lors du traitement de la réponse");
    } finally {
      // DÉBLOQUER les changements de question SEULEMENT à la fin
      setIsProcessingAnswer(false);
      console.log("🔓 [QUIZ] Traitement terminé, changements de questions réactivés");
    }
